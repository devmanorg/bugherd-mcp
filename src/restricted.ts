...
type TaskScope = "taskboard" | "all" | "feedback" | "archived";
...
function sortTasks<T extends { created_at: string; updated_at: string; local_task_id: number; id: number }>(
  tasks: T[],
  mode: SortMode,
): T[] {
  if (mode === "api") return tasks;
...
function encodePageCursor(value: { page: number }): string {
  return Buffer.from(JSON.stringify(value), "utf8").toString("base64url");
}

function decodePageCursor(cursor: string): { page: number } {
  const raw = Buffer.from(cursor, "base64url").toString("utf8");
  const parsed = JSON.parse(raw) as unknown;
  return z.object({ page: z.number().int().positive() }).parse(parsed);
}

function encodeTextCursor(value: { offset: number }): string {
  return Buffer.from(JSON.stringify(value), "utf8").toString("base64url");
}

function decodeTextCursor(cursor: string): { offset: number } {
  const raw = Buffer.from(cursor, "base64url").toString("utf8");
  const parsed = JSON.parse(raw) as unknown;
...
function formatColumnLine(col: { id: number; name: string; position?: number | null }, isActive: boolean): string {
...
const TOOLS = [
  ...
  },
  {
    name: "tasks_list",
    description:
      "List tasks for the configured project. Returns up to 30 items with pagination.",
    inputSchema: {
  ...
  },
  {
    ...
    description:
      "Aux tool: read long task description in chunks. Returns next_cursor when more is available.",
    inputSchema: {
      type: "object" as const,
      properties: {
  ...
  },
  {
    ...
    description:
      "Aux tool: read a long comment text in chunks. Returns next_cursor when more is available.",
    inputSchema: {
      type: "object" as const,
      properties: {
        local_task_id: { type: "number" },
    ...
    switch (name) {
      case "columns_list": {
        ...
        const lines = cols.map((c) =>
          formatColumnLine(c, activeSet ? activeSet.has(c.id) : false),
        );
      ...
      case "tasks_list": {
        ...
        const scope: TaskScope = parsed.scope ?? "taskboard";
        ...
        if (parsed.tag) {
          ...
          tasks = tasks.filter((t) =>
            (t.tag_names ?? []).some((tag) => tag.toLowerCase() === target),
          );
        }
        ...
        if (env.activeColumnIds && (scope === "taskboard" || scope === "all")) {
          ...
          tasks = tasks.filter((t) =>
            typeof t.status_id === "number" ? allowed.has(t.status_id) : true,
          );
        }

        tasks = sortTasks(tasks as any, parsed.sort) as any;
        ...
        const nextPage =
          totalPages && currentPage < totalPages ? currentPage + 1 : null;
        ...
        if (totalPages) headerParts.push(`total_pages: ${totalPages}`);
      ...
      case "task_get": {
        ...
        const screenshot =
          task.screenshot_url ?? task.screenshot ?? "No screenshot";
        ...
        const { chunk: descChunk, nextCursor: descNextCursor } = textChunk(
          task.description,
          0,
          previewMaxChars,
        );
        ...
        if (task.fullstory_session_url) techLines.push(`fullstory: ${task.fullstory_session_url}`);
        if (task.logrocket_session_url) techLines.push(`logrocket: ${task.logrocket_session_url}`);
      ...
      case "task_description_more": {
        ...
        const { chunk, nextCursor } = textChunk(
          task.description,
          offset,
          env.descriptionMaxChars,
        );
          ...
          content: [
            {
              type: "text" as const,
              text:
                `## Task #${task.local_task_id} Description (chunk)\n` +
                `offset: ${offset}\n` +
                (nextCursor ? `next_cursor: ${nextCursor}\n` : "") +
                `\n${chunk}`,
            },
          ...
          content: [
            {
              type: "text" as const,
              text:
                `## Comments on Task #${task.local_task_id} (${slice.length}/${total})\n` +
                `page: ${page}\n` +
                `cursor: ${cursor}\n` +
                (nextCursor ? `next_cursor: ${nextCursor}\n` : "") +
                (prevCursor ? `prev_cursor: ${prevCursor}\n` : "") +
          ...
          content: [
            {
              type: "text" as const,
              text:
                `## Comment ${comment.id} on Task #${task.local_task_id} (chunk)\n\n` +
                `author: ${author}\n` +
...
server
  .connect(transport)
  .then(() => {
    console.error("BugHerd MCP Restricted Server running on stdio");
  })
  .catch((error) => {
